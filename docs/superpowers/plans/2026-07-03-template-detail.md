# Workflow Template Detail & Edit Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a workflow template detail page under `/templates/[id]` that shows a read-only detailed view with a status toggle, check for active running instances, and enables an interactive edit mode to modify the template using `PUT /api/templates/:id`.

**Architecture:** Use Next.js dynamic routing with App Router (`templates/[id]/page.tsx`). Fetch template data, agents, events, and all running instances to determine the locked status. If editable, toggles to an interactive form allowing steps addition, deletion, and property updates.

**Tech Stack:** Next.js 16 (App Router), React 19, Tailwind CSS v4, Lucide icons (or inline SVGs).

## Global Constraints

- Modern cyber-glassmorphism dark-mode UI with high-fidelity components, micro-interactions, responsive grid layout, and clean transitions.
- Client-side validation for template steps before submitting.
- REST API integration with localhost:3000 backend.

---

### Task 1: Add Link to Detail Page in Template Card

**Files:**
- Modify: `dashboard/src/app/templates/page.tsx:86-99`

**Interfaces:**
- Consumes: None
- Produces: Dynamic link to `/templates/[id]`

- [ ] **Step 1: Modify templates/page.tsx to render a Details link**

Modify the bottom section of the template cards to include a "Details →" link pointing to `/templates/${tpl.id}`:
...
- [ ] **Step 2: Run a next build to verify the change compiles**

Run: `npm run build` in directory `dashboard/`
Expected: Next build compiles successfully.

- [ ] **Step 3: Commit the changes**

```bash
git add dashboard/src/app/templates/page.tsx
git commit -m "feat(frontend): add details link to template configuration card"
```

---

### Task 2: Implement Template Detail and Edit Page `/templates/[id]/page.tsx`

**Files:**
- Create: `dashboard/src/app/templates/[id]/page.tsx`

**Interfaces:**
- Consumes: `GET /api/templates/:id`, `GET /api/all-instances`, `GET /api/agents`, `GET /api/events`, `PUT /api/templates/:id`, `PATCH /api/templates/:id/status`
- Produces: Details and interactive editing page for a specific template.

- [ ] **Step 1: Create the template detail page component**

Create `dashboard/src/app/templates/[id]/page.tsx` with dynamic page parameters handling for Next.js 16/15 and full editing/viewing states. We use `use` from React to unwrap the dynamic `params` Promise:
  ...
  if (error && !template) {
    return (
      <div className="bg-rose-500/10 border border-rose-500/30 text-rose-400 p-4 rounded-lg text-sm font-medium">
        {error}
        <div className="mt-4">
          <Link href="/templates" className="text-xs underline">
            Back to Templates
          </Link>
        </div>
      </div>
...
- [ ] **Step 2: Run a next build to verify the detail page compiles**
...
```bash
git add dashboard/src/app/templates/\[id\]/page.tsx
git commit -m "feat(frontend): implement template detail & edit dynamic page"
```
...
**Files:**
- Test: Manual verify from the browser.

- [ ] **Step 1: Restart next app if needed and click "Details" link**

Verify that on the Configuration page (`/templates`), each template card now shows a "Details" link. Click on one of the cards to verify that it loads the detail page under `/templates/[id]` successfully.

- [ ] **Step 2: Test Editing Mode**

Click "Edit Template", update the description or sequence of steps, and click "Save Changes". Verify that it successfully updates and saves.

- [ ] **Step 3: Test Locked Warning Alert**

Go to `/trigger`, choose a booking and trigger a workflow process. Confirm that a workflow instance is active on the Home overview page. Go to `/templates`, click the template details, and verify that:
1. The locked yellow banner/warning is shown.
2. The "Edit Template" button is disabled.
