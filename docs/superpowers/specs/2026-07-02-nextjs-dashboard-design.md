...
1.  **Template Configuration**: User can view templates, toggle activation status, and create new templates with sequential steps (mapping to roles or specific user IDs).
...
*   **List View**: Interactive grid of templates. Displays Name, Description, Trigger Event, Step Count, and an active status badge with a Toggle Switch (`PATCH /api/templates/:id/status`).
...
2.  **`GET /api/templates`**:
    Returns all templates: `id`, `name`, `description`, `trigger_event`, `is_active`, and their nested `steps` (sorted by `sequence`).

3.  **`GET /api/instances`**:
    Returns all instances from `workflow_instances` with initiator details, current state of steps, audit log trail, and source entity information.
